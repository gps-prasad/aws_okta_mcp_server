...
def register_user_tools(server: FastMCP, okta_client: OktaMcpClient):
    ...
    async def list_okta_users(
        ...
        try:
            ...
            users = await get_dummy_data("users")
            err = None
            resp = None
    ...
    async def get_okta_user(
        ...
        try:
            ...
            users = await get_dummy_data("users")
            user = next((u for u in users if u.get('profile', {}).get('login') == user_id or u.get('id') == user_id), None)
            err = None
    ...
    async def list_okta_user_groups(
        ...
        try:
            ...
            result = {
                "groups": [group for group in groups] if groups else [],
    ...
    async def list_okta_user_applications(
        ...
        try:
            ...
            users = await get_dummy_data("users")
            user = next((u for u in users if u.get('profile', {}).get('login') == user_id or u.get('id') == user_id), None)
            app_links = user.get('applications', []) if users else []
            resp = None
            err = None
            ...
            result = {
                "app_links": [app_link for app_link in app_links] if app_links else [],
    ...
    async def list_okta_user_factors(
        ...
        try:
            ...
            result = {
                "factors": [factor for factor in factors] if factors else [],
